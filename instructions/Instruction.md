...
- **Not all columns are always visible**
  For now it is possible that some columns will not be visible when some are too wide. There are 2 workarounds for now - View can be scrolled via horizontal scroll bar (1 on image) - Size of other columns can be slimmed (2)
...
Example: there are 4 checked folders which _may_ be empty `/cow/`, `/cow/ear/`, `/cow/ear/stack/`, `/cow/ear/flag/`.
